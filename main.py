import math
import random

import pygame

from src.core.constants import PONTOS_POR_METEORO_ESPECIAL, PONTOS_POR_METEORO_NORMAL
from src.core.constants import (
    LARGURA, ALTURA,
    PRETO, BRANCO, VERDE, VERMELHO,
)
from src.core.tela import Tela
from src.components.jogador import Jogador
from src.components.tiros import Tiro
from src.components.inimigos import MeteoroNormal, MeteoroEspecial, FragmentoMeteoro, ImpactoTiro, Boss

pygame.init()

tela = Tela()
clock = pygame.time.Clock()

fonte_placar = pygame.font.SysFont("Arial", 25, bold=True)
fonte_grande = pygame.font.SysFont("Arial", 60, bold=True)
fonte_media = pygame.font.SysFont("Arial", 35, bold=True)
fonte_horda = pygame.font.SysFont("Arial", 45, bold=True)

HORDAS_POR_BOSS = 3


def duracao_horda(n):
    return int(500 * (1 + 0.5 * math.log(n)))


def intervalo_spawn(horda):
    return max(300, int(1000 / (1 + 0.4 * math.log(horda))))


def hp_boss(ciclo):
    return int(600 + 300 * math.log(ciclo + 1)) if ciclo > 0 else 600


todos_sprites = pygame.sprite.Group()
grupo_inimigos = pygame.sprite.Group()
grupo_tiros = pygame.sprite.Group()
grupo_tiros_inimigos = pygame.sprite.Group()

pontos = 0
vidas = 3
fase_boss = False
game_over = False
chefao = None

horda_atual = 0
proxima_horda = 1
pontos_inicio_horda = 0
pausa_ate = 0
mensagem_horda = ""
mensagem_ate = 0
ciclo_boss = 0
pontos_arma = 0


def get_horda():
    return max(1, horda_atual)


def resetar_jogo():
    global pontos, vidas, fase_boss, game_over, player, chefao
    global horda_atual, proxima_horda, pontos_inicio_horda
    global pausa_ate, mensagem_horda, mensagem_ate, ciclo_boss, pontos_arma
    pontos = 0
    vidas = 3
    fase_boss = False
    game_over = False
    chefao = None
    horda_atual = 0
    proxima_horda = 1
    pontos_inicio_horda = 0
    pausa_ate = 0
    mensagem_horda = ""
    mensagem_ate = 0
    ciclo_boss = 0
    pontos_arma = 0
    todos_sprites.empty()
    grupo_inimigos.empty()
    grupo_tiros.empty()
    grupo_tiros_inimigos.empty()
    player = Jogador()
    todos_sprites.add(player)


player = Jogador()
todos_sprites.add(player)

SPAWN_TIMER = pygame.USEREVENT + 1
pygame.time.set_timer(SPAWN_TIMER, 1000)

ultimo_tiro = 0
COOLDOWN_TIRO = 180

rodando = True
while rodando:
    clock.tick(60)
    agora = pygame.time.get_ticks()

    for evento in pygame.event.get():
        if evento.type == pygame.QUIT:
            rodando = False

        if game_over and evento.type == pygame.KEYDOWN:
            if evento.key == pygame.K_r:
                resetar_jogo()

        if not game_over:
            if evento.type == SPAWN_TIMER and not fase_boss and horda_atual > 0 and pausa_ate == 0:
                sorteio = random.random()
                hp_v = 2 + int(0.5 * math.log(horda_atual + 1))
                hp_e = 12 + int(3 * math.log(horda_atual + 1))
                prob_especial = min(0.55, 0.05 + 0.08 * math.log(horda_atual + 1))
                if sorteio < prob_especial:
                    novo = MeteoroEspecial(hp_e, get_horda)
                else:
                    novo = MeteoroNormal(hp_v, get_horda)
                todos_sprites.add(novo)
                grupo_inimigos.add(novo)

    if not game_over:
        # Shooting
        if pygame.key.get_pressed()[pygame.K_SPACE] and agora - ultimo_tiro >= COOLDOWN_TIRO:
            ultimo_tiro = agora
            num = (pontos_arma // 500) + 1
            dano_at = 2 if pontos_arma >= 1000 else 1
            esc_at = 1 + (0.5 * (min(pontos_arma, 900) // 300))
            for i in range(num):
                off = (i - (num - 1) / 2) * 20
                t = Tiro(player.rect.centerx + off, player.rect.top, dano_at, esc_at)
                todos_sprites.add(t)
                grupo_tiros.add(t)

    if not game_over:
        if not fase_boss:
            horda_completa = (horda_atual == 0) or (pontos >= pontos_inicio_horda + duracao_horda(horda_atual))
            if horda_completa and pausa_ate == 0:
                if horda_atual > 0 and horda_atual % HORDAS_POR_BOSS == 0:
                    fase_boss = True

                    for s in grupo_inimigos:
                        s.kill()

                    chefao = Boss(todos_sprites, grupo_tiros_inimigos, hp_boss(ciclo_boss))
                    todos_sprites.add(chefao)
                    grupo_inimigos.add(chefao)

                else:

                    pausa_ate = agora + 2000
                    mensagem_horda = f"INICIANDO HORDA {proxima_horda}"

                    mensagem_ate = agora + 4000


            if pausa_ate > 0 and agora >= pausa_ate:
                pausa_ate = 0
                horda_atual = proxima_horda
                proxima_horda += 1
                pontos_inicio_horda = pontos
                pygame.time.set_timer(SPAWN_TIMER, intervalo_spawn(horda_atual))

        todos_sprites.update()

        hits = pygame.sprite.groupcollide(grupo_inimigos, grupo_tiros, False, True)
        for inimigo, balas in hits.items():
            total_dano = sum(b.dano for b in balas)
            if isinstance(inimigo, Boss):
                inimigo.vida -= total_dano
                if inimigo.vida <= 0:
                    inimigo.kill()
                    fase_boss = False
                    ciclo_boss += 1
                    pontos_arma = 0
                    pausa_ate = agora + 2000
                    mensagem_horda = f"INICIANDO HORDA {proxima_horda}"
                    mensagem_ate = agora + 4000
            else:
                inimigo.vida -= total_dano
                if isinstance(inimigo, (MeteoroNormal, MeteoroEspecial)):
                    for b in balas:
                        todos_sprites.add(ImpactoTiro(b.rect.centerx, b.rect.centery))
                if inimigo.vida <= 0:
                    if isinstance(inimigo, MeteoroNormal):
                        for _ in range(random.randint(4, 7)):
                            f = FragmentoMeteoro(inimigo.rect.centerx, inimigo.rect.centery, inimigo.tom)
                            todos_sprites.add(f)

                    ganho = PONTOS_POR_METEORO_ESPECIAL if isinstance(inimigo, MeteoroEspecial) else PONTOS_POR_METEORO_NORMAL

                    pontos += ganho

                    pontos_arma += ganho
                    inimigo.kill()

                elif isinstance(inimigo, (MeteoroNormal, MeteoroEspecial)):
                    inimigo.atualizar_visual()

        hits_defesa = pygame.sprite.groupcollide(grupo_tiros_inimigos, grupo_tiros, False, True)
        for p, balas in hits_defesa.items():
            if hasattr(p, 'vida'):
                p.vida -= sum(b.dano for b in balas)
                if p.vida <= 0:
                    p.kill()

        if not player.invencivel:
            if pygame.sprite.spritecollideany(player, grupo_inimigos) or pygame.sprite.spritecollideany(player, grupo_tiros_inimigos):
                vidas -= 1
                if vidas <= 0:
                    game_over = True
                else:
                    player.invencivel = True
                    player.tempo_invencivel = agora + 5000

    tela.preencher(PRETO)
    if not game_over:
        tela.desenhar(todos_sprites)
        tela.blit(fonte_placar.render(f"SCORE: {pontos}", True, BRANCO), (20, 20))
        tela.blit(fonte_placar.render(f"LIVES: {vidas}", True, VERMELHO), (20, 50))
        tela.blit(fonte_placar.render(f"WEAPON LVL: {(pontos_arma // 500) + 1}", True, VERDE), (20, 80))
        if horda_atual > 0:
            label = f"BOSS {ciclo_boss + 1}" if fase_boss else f"HORDA: {horda_atual}"
            tela.blit(fonte_placar.render(label, True, BRANCO), (20, 110))

        if fase_boss and chefao is not None and chefao.alive():
            barra_w = 200
            vida_w = int(barra_w * chefao.vida / chefao.vida_max)
            pygame.draw.rect(tela.surface, VERMELHO, (LARGURA // 2 - 100, 20, barra_w, 20))
            pygame.draw.rect(tela.surface, VERDE, (LARGURA // 2 - 100, 20, max(0, vida_w), 20))

        if mensagem_horda and agora < mensagem_ate:
            txt = fonte_horda.render(mensagem_horda, True, VERMELHO)
            tela.blit(txt, (LARGURA // 2 - txt.get_width() // 2, ALTURA // 2 - txt.get_height() // 2))

    else:
        txt_g = fonte_grande.render("GAME OVER", True, VERMELHO)
        txt_s = fonte_media.render(f"SCORE: {pontos}", True, BRANCO)
        txt_h = fonte_media.render(f"HORDA: {horda_atual}", True, BRANCO)
        txt_r = fonte_media.render("R - Reiniciar", True, BRANCO)
        tela.blit(txt_g, (LARGURA // 2 - txt_g.get_width() // 2, ALTURA // 2 - 100))
        tela.blit(txt_s, (LARGURA // 2 - txt_s.get_width() // 2, ALTURA // 2 - 20))
        tela.blit(txt_h, (LARGURA // 2 - txt_h.get_width() // 2, ALTURA // 2 + 30))
        tela.blit(txt_r, (LARGURA // 2 - txt_r.get_width() // 2, ALTURA // 2 + 80))

    tela.atualizar()

pygame.quit()
